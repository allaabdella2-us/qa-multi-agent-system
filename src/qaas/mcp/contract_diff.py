...
class _CrossOriginRedirect(Exception):
    """A redirect that would leave the origin the spec was allowed from."""
...
class _SameOriginRedirects(urllib.request.HTTPRedirectHandler):
    ...
    `_read_spec` checks the URL it is *handed* against the application under
    test, and `urlopen` then followed any redirect to any host -- so a target
    answering `/openapi.json` with a 302 to somewhere else had this process
    fetch that somewhere else and diff it as though it were the target's spec.
    The origin check was a check on the first hop only.
    """
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):  # noqa: ANN001 - urllib's signature
        if _origin(newurl) != _origin(req.full_url):
            fp.close()
            raise _CrossOriginRedirect(newurl)
        return super().redirect_request(req, fp, code, msg, headers, newurl)


_SPEC_OPENER = urllib.request.build_opener(_SameOriginRedirects)


def _fetch_spec(url: str) -> tuple[dict[str, Any] | None, str | None]:
    try:
        with _SPEC_OPENER.open(url, timeout=FETCH_TIMEOUT_S) as resp:  # noqa: S310 - http(s) only, checked by caller
            body = resp.read().decode(errors="replace")
    except _CrossOriginRedirect as exc:
        return None, (
            f"{url} redirected to {exc.target}, a different origin. A spec is only "
            "read from the application under test, so the redirect was not followed."
        )
...
def _consumer_hits(root: Path, target_root: Path, terms: list[str]) -> list[dict[str, Any]]:
    """Every line under `root` naming one of `terms`, up to MAX_CONSUMER_HITS."""
    ...
    for file in _walk_sources(root):
        ...
        for number, line in enumerate(text.splitlines(), start=1):
            if any(term in line for term in terms):
                hits.append({
                    "file": str(file.relative_to(target_root)) if file.is_relative_to(target_root) else str(file),
                    "line": number,
                    "text": line.strip()[:200],
                })
...
#: Two ways this produced false evidence, both fixed in the template below.
#: The login body was hard-coded `{"email": ..., "password": ...}` whatever the
#: profile's `username_field`/`password_field` said -- and the token was read
#: from `access_token` whatever `token_path` said -- so against an API with
#: other names every generated test failed at its `token` fixture, and the
#: envelope cited a login failure as a contract violation. And the anonymous-
#: access test accepted only 401, so an endpoint correctly refusing with 403
#: (FastAPI `HTTPBearer`'s default) was reported as an auth *bypass*.
...
LOGIN_USER_FIELD = {login_user_field!r}
LOGIN_PASSWORD_FIELD = {login_password_field!r}
...
def token():
    ...
    status, body = _call(
        LOGIN_PATH, "POST", body={{LOGIN_USER_FIELD: LOGIN_EMAIL, LOGIN_PASSWORD_FIELD: LOGIN_PASSWORD}}
    )
    ...
    assert token, "the login response has no token at %s: %s" % (TOKEN_PATH, body[:300])
    return token
...
def test_endpoint_requires_authentication():
    ...
    # 401 or 403: both refuse a caller with no credentials, and 403 is what
    # FastAPI's HTTPBearer answers by default. Anything else let them in.
...
def build_tools(ctx: ToolContext) -> list:
    ...
    _auth = getattr(_profile, "auth", None) if _profile else None
    ...
    async def diff_openapi(args: dict[str, Any]) -> dict[str, Any]:
        ...
        # Off the event loop. `_fetch_spec` is a synchronous `urlopen` with a
        # ten-second timeout, and inside an async handler it stalled every
        # other agent's tool calls for as long as the target took to answer.
        doc_a, problem = await asyncio.to_thread(_read_spec, ref_a, ctx.target_root, ctx)
        if problem:
            return err(f"Could not load spec_a: {problem}")
        doc_b, problem = await asyncio.to_thread(_read_spec, ref_b, ctx.target_root, ctx)
        if problem:
            hint = (
    ...
    async def find_consumers(args: dict[str, Any]) -> dict[str, Any]:
        ...
        # Off the event loop: `os.walk` and a read of every source file in the
        # repository ran inline in this async handler, and on a real monorepo
        # that stalled every other agent's tool calls until it finished.
        hits = await asyncio.to_thread(_consumer_hits, root, ctx.target_root, terms)
    ...
    async def generate_contract_test(args: dict[str, Any]) -> dict[str, Any]:
        ...
        doc, problem = await asyncio.to_thread(_read_spec, ref, ctx.target_root, ctx)
        if problem:
        ...
        source = TEST_TEMPLATE.format(
            ...
            login_user_field=getattr(_auth, "username_field", None) or "email",
            login_password_field=getattr(_auth, "password_field", None) or "password",
            token_path=getattr(_auth, "token_path", None) or "access_token",
        ...
        return ok(
            ...
            + (", and that anonymous calls get 401 or 403" if _requires_auth(op, doc) else "")
